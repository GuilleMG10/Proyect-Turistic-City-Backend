...
4. **Cache con Redis** para consultas frecuentes
5. **Validación más robusta** con biblioteca como validator
6. **Tests unitarios** y de integración
7. **Soft delete para eventos** (agregar columna `active`)
8. **Healthcheck endpoint** más completo (verificar DB, IA, etc.)
9. **Metrics y monitoring** con Prometheus
10. **Documentación Swagger** para la API

### Seguridad
1. **Rate limiting** por IP y por usuario
2. **Validación de entrada** más estricta
3. **CORS más restrictivo** en producción
4. **Rotación de JWT** con refresh tokens
5. **Audit logging** de operaciones administrativas
