...
export default function MobileMenu({ isOpen, onClose, links }: MobileMenuProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 md:hidden flex items-center justify-center">
      ...
      {/* Menu Content */}
      <div className="relative w-full max-w-lg px-0 py-0 flex flex-col h-full max-h-[80vh]">
            ...
            {links.map((link) => (
              <li key={link.to}>
                <Link
                  to={link.to}
                  onClick={onClose}
                  className="flex items-center justify-between py-4 text-gray-300 hover:text-white hover:bg-white/10 rounded-2xl transition-all group">
