...
export default function FilterModal({ isOpen, onClose, onApplyFilters, currentFilters, availableCategories }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4" role="dialog" aria-modal="true" aria-labelledby="filter-modal-title">
      <div className="bg-white rounded-2xl max-w-md w-full max-h-[90vh] overflow-hidden" role="document">
        ...
        <section className="p-4 overflow-y-auto max-h-96 space-y-6" aria-label="Opciones de filtro">
          {/* Categories */}
          <fieldset>
            <legend className="font-medium mb-3">Categorías</legend>
            <div id="categories-desc" className="sr-only">
              Selecciona una o más categorías para filtrar los resultados
            </div>
            <ul className="space-y-2" aria-describedby="categories-desc">
                ...
                <li key={category}>
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      ...
                      aria-describedby={`category-${category}-help`}
                    />
                    {category}
                    <span id={`category-${category}-help`} className="sr-only">
                      {filters.categories.includes(category) ? 'Seleccionado' : 'No seleccionado'}
                    </span>
                  </label>
                </li>
              ))}
        ...
        <footer className="border-t p-4 flex gap-3">
          <button
            onClick={handleClear}
            className="flex-1 border border-gray-300 py-2 px-4 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          >
            Aplicar Filtros
            <span id="apply-filters-desc" className="sr-only">
              Aplicar los filtros seleccionados y cerrar la ventana
            </span>
          </button>
        </footer>
      </div>
