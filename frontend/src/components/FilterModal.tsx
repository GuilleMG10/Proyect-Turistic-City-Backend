...
export default function FilterModal({ isOpen, onClose, onApplyFilters, currentFilters }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" role="dialog" aria-modal="true" aria-labelledby="filter-modal-title">
      <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-md w-full max-h-[90vh] overflow-hidden" role="document">
        {/* Header */}
        <header className="border-b dark:border-gray-700 p-4 flex items-center justify-between">
          <h2 id="filter-modal-title" className="text-lg font-semibold text-gray-900 dark:text-white">Filtros</h2>
          <button
            onClick={onClose}
            className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-gray-100"
        ...
        <section className="p-4 overflow-y-auto max-h-96 space-y-6" aria-label="Opciones de filtro">
          {/* Zones */}
          <fieldset>
            <legend className="font-medium mb-3 text-gray-900 dark:text-white">Zona</legend>
          ...
          {/* Age Range */}
          <fieldset>
            <legend className="font-medium mb-3 text-gray-900 dark:text-white">Rango de Edad</legend>
          ...
          {/* Price Range */}
          <fieldset>
            <legend className="font-medium mb-3 text-gray-900 dark:text-white">Rango de Precio</legend>
          ...
          <fieldset>
            <legend className="font-medium mb-3 text-gray-900 dark:text-white">Calificación Mínima</legend>
        ...
        {/* Footer */}
        <footer className="border-t dark:border-gray-700 p-4 flex gap-3">
          <button
            type="button"
            onClick={handleClear}
            className="flex-1 border-2 border-gray-300 dark:border-gray-600 py-2.5 px-4 rounded-lg font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-400 dark:hover:border-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
          >
            Limpiar
          </button>
          <button
