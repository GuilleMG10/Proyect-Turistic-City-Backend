    ...
    <>
      ...
      >
        ...
        <header className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 p-4 flex items-center justify-between">
          <h2 id="event-modal-title" className="text-xl font-semibold text-gray-900 dark:text-white">{event.name}</h2>
          <button
            onClick={onClose}
        ...
        <section className="p-6 space-y-6">
          ...
          <aside className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium text-gray-900 dark:text-white"><time dateTime={event.event_date}>{formattedDate}</time></p>
                <p className="text-sm text-gray-600 dark:text-gray-400"><time dateTime={event.event_date}>{formattedTime}</time></p>
              </div>
            </div>
          ...
          <section>
            ...
            <address className="text-gray-700 dark:text-gray-300 not-italic">{event.location}</address>
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <article className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
              ...
              <p className="font-semibold text-gray-900 dark:text-white">Bs {event.price}</p>
            </article>
          ...
          <footer className="flex gap-3 pt-4">
            {isAdmin ? (
              ...
              <>
                ...
                <button 
                  className={`${(user && event.status !== 'finished') ? 'flex-1' : 'w-full'} border border-gray-300 dark:border-gray-600 px-4 py-2 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-gray-100`}
